...
class PromptGenerator:
    ...
    def __init__(self, db_path: str, max_tags: int = 15, use_space_separator: bool = True, api_client: Optional[APIClient] = None, semantic_tagger: Optional[Any] = None):
        """
        初始化提示词生成器
    ...
    def _generate_semantic_then_select(self, description: str) -> str:
        ...
        try:
            ...
            tags_string, results = self.semantic_tagger.search(  # type: ignore[union-attr]
                description, top_k=search_top_k, limit=search_limit, popularity_weight=search_pop_w
            )
        ...
        if self.is_api_enabled():
            ...
            try:
                raw_selected = self.api_client.select_tags(description, tags_string, self.max_tags)  # type: ignore[union-attr]
                
                # 验证：只保留候选列表中存在的标签
                candidate_set = {t.strip().lower() for t in tags_string.split(',') if t.strip()}
                selected_tags = []
                ...
                try:
                    validated_tags = self.api_client.validate_tags(description, selected_tags)  # type: ignore[union-attr]
                    if validated_tags != selected_tags:
                        print(f"[Generator] 验证后删除 {len(selected_tags) - len(validated_tags)} 个无关标签")
...
def create_generator(db_path: str, max_tags: Optional[int] = None, use_space_separator: bool = True, api_client: Optional[APIClient] = None, semantic_tagger: Optional[Any] = None) -> PromptGenerator:
