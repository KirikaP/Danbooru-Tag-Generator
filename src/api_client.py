...
class APIClient:
    ...
    def __init__(
        ...
        self.thinking_select_tags = thinking_cfg.get(
            "select_tags", self.enable_thinking
        )
        ...
        self.session: Any = (
            ...
            if REQUESTS_AVAILABLE and requests_lib is not None
            else None
        )
    ...
    def _raise_if_cancelled(self) -> None:
        if self.cancel_event is not None and self.cancel_event.is_set():
            raise InterruptedError("用户中断了当前生成")
    ...
    def _post_with_cancel(self, url: str, headers: dict, data: dict, timeout: int):
        self._raise_if_cancelled()
        if self.session is None:
            raise RuntimeError("requests session 不可用")
    ...
    def _request_with_retry(
        self, url: str, headers: dict, data: dict, timeout: int
    ) -> Any:
        """带重试的 POST 请求
        ...
        for attempt in range(max_retries + 1):
            self._raise_if_cancelled()
            try:
                response = self._post_with_cancel(url, headers, data, timeout)
                if response.status_code in retry_on_status and attempt < max_retries:
                    wait = backoff_factor**attempt
                    print(
                        f"[APIClient] 状态码 {response.status_code}，{wait:.1f}秒后重试 "
                ...
                if attempt < max_retries:
                    wait = backoff_factor**attempt
                    print(
                        f"[APIClient] 请求异常: {exc}，{wait:.1f}秒后重试 "
                        f"(第{attempt + 1}/{max_retries}次)..."
                    )
                    self._sleep_with_cancel(wait)
                else:
    ...
    def generate(self, prompt: str, user_prompt: Optional[str] = None) -> str:
        ...
        if not self.is_available():
            raise RuntimeError(
                "API客户端不可用，请检查是否安装了requests库并配置了API密钥"
            )
    ...
    def select_tags(
        self, description: str, candidate_tags: str, max_tags: int = 25
    ) -> str:
        ...
        select_temperature = select_cfg.get(
            "temperature", cfg.get("select_tags_temperature")
        )
        select_top_p = select_cfg.get("top_p", cfg.get("select_tags_top_p"))
        select_max_tokens = select_cfg.get(
            "max_tokens", cfg.get("select_tags_max_tokens")
        )
        select_tags_max = cfg.get("select_tags_max")

        # 获取 system prompt 并替换占位符
        base_system = str(cfg.get("system_prompt") or "")
        ...
        start = time.time()
        label = (
            "正在思考并筛选标签..." if self.thinking_select_tags else "正在筛选标签..."
        )
        response = self._call_with_spinner(
            label,
