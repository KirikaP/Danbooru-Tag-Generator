...
class SemanticTagger:
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            ...
            cls._instance.cancel_event = None
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if response.status_code in retry_on_status and attempt < max_retries:
                    ...
                    print(
                        f"[SemanticTagger] 状态码 {response.status_code}，{wait:.1f} 秒后重试 "
                ...
                if attempt < max_retries:
                    wait = backoff_factor**attempt
                    print(
                        f"[SemanticTagger] 请求异常: {exc}，{wait:.1f} 秒后重试 "
                        f"(第{attempt + 1}/{max_retries}次)..."
                    )
                    self._sleep_with_cancel(wait)
                else:
    ...
    def load(self):
        ...
        if os.path.exists(new_cache_path):
            try:
                ...
                print(
                    f"[SemanticTagger] Loaded NEW embeddings cache ({len(self._cache_names)} tags)"
                )
                return
        ...
        print(
            f"[SemanticTagger] Ready (Embedding API: {self.embedding_api_url}, Model: {self.embedding_model}; Reranker API: {self.reranker_api_url}, Model: {self.reranker_model})"
        )
        ...
        if os.path.exists("cache/embeddings_cache.pkl"):
            ...
            self._cache_nsfw = data.get("nsfw", [])
            print(
                f"[SemanticTagger] 缓存构建完成，已加载 ({len(self._cache_names)} 个标签)"
            )
        ...
        for batch_idx, i in enumerate(range(0, len(texts), batch_size)):
            ...
            try:
                ...
                if len(texts) > 100:  # 只有文本较多时才输出进度
                    print(
                        f"    进度: [{batch_idx + 1}/{total_batches}] 已编码 {min(i + batch_size, len(texts))}/{len(texts)} 个文本 (耗时: {batch_time:.2f}秒)"
                    )
    ...
    def _build_embeddings_cache(self, save_path: str):
        ...
        self.emb_cn_core = self._get_embedding(
            self.df["cn_core"].tolist(), batch_size=32
        )
    ...
    def _smart_split(self, text: str) -> List[str]:
        ...
        tokens = [
            t.strip()
            for t in tokens
            if t.strip() and re.search(r"[\u4e00-\u9fa5a-zA-Z]", t)
        ]
    ...
    def _realtime_search(
        ...
        try:
            return self._direct_semantic_match(
                query, valid_tags, limit, popularity_weight
            )
        except InterruptedError:
            raise
        except Exception as e:
    ...
    def _keyword_filter(self, query: str, limit: int = 200) -> List[Dict]:
        ...
        for idx, row in self.df.iterrows():
            ...
            if tag_name in ["1girl", "1boy", "solo", "cat_girl", "cat_ears"]:
                if any(
                    kw in tag_name.lower() or kw in cn_name.lower() for kw in kw_set
                ):
                    score += 50
    ...
    def _semantic_rerank(
        ...
        try:
            ...
            valid_tags = [
                t
                for t in sorted_tags[:limit]
                if self._tag_exists(t["tag"])
                and t.get("semantic_score", 0) >= similarity_threshold
            ]
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if attempt < max_retries:
                    ...
                    print(
                        f"[SemanticTagger] Reranker retry {attempt + 1}/{max_retries} after {wait:.1f}s: {e}"
                    )
                    self._sleep_with_cancel(wait)
                else:
                    raise
